...
import os
api_key = os.getenv("grok_api") or os.getenv("GROQ_API_KEY")
        ...
        try:
            ...
            chat_completion = client.chat.completions.create(
                messages=[
                    {
                        "role": "system",
                        "content": "You are a helpful assistant that answers questions based on document excerpts. Always cite the specific filename when providing information. Be direct and natural."
                    },
                    {
                        "role": "user",
                        "content": prompt,
            ...
            pass
    parts = []
    for m in relevant_matches[:5]:
        parts.append(f"{m['text']} (from {m['filename']})")
    return " ".join(parts)
