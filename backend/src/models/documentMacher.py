...
def _get_st_model() -> tuple[_t.Any, _t.Any]:
    ...
    if _ST_MODEL is None:
        logger.info("Loading sentence-transformers matcher model...")
        os.environ['PYTORCH_ENABLE_MPS_FALLBACK'] = '1'
        _ST_MODEL = ModelHub.get_matcher_encoder()
...
def _pdf_tables(data: bytes) -> dict[str, _t.Any]:
    """Extract PDF tables as DataFrames with header inference."""
    import pandas as pd
    tables: dict[str, _t.Any] = {}
    try:
        import pdfplumber
    except Exception:
        return tables
    with pdfplumber.open(io.BytesIO(data)) as pdf:
        for p_idx, page in enumerate(pdf.pages, 1):
            ...
            for raw in page_tables:
                ...
                tables[f"Page {p_idx} • Table {count}"] = df
            
            # OCR fallback for scanned pages (best-effort)
            try:
                import fitz
                from PIL import Image
                import pytesseract
                doc = fitz.open(stream=data, filetype="pdf")
                pix = doc.load_page(p_idx - 1).get_pixmap(dpi=200)
                ...
                for line in ocr_text.splitlines():
                    if "\t" in line:
                        ...
                        import re as _re
                        cols = [c.strip() for c in _re.split(r"\s{2,}", line) if c.strip()]
                    if len(cols) >= 2:
                        rows.append(cols)
                ...
                if rows:
                    ...
                    if all(h.strip() for h in header):
                        ...
                        df = pd.DataFrame(rows)
                        df.columns = [f"col_{i+1}" for i in range(len(df.columns))]
                    tables[f"Page {p_idx} • OCR Table {count + 1}"] = df
            except Exception:
                pass
    return tables
...
def _docx_tables(data: bytes) -> dict[str, _t.Any]:
    """Extract DOCX tables as DataFrames with header inference."""
    import pandas as pd
    from docx import Document
    doc = Document(io.BytesIO(data))
    tables: dict[str, _t.Any] = {}
    for idx, tbl in enumerate(doc.tables, 1):
        ...
        if not rows:
            continue
        # Infer headers from first row if non-empty
        header = rows[0]
        if all(h.strip() for h in header):
            df = pd.DataFrame(rows[1:], columns=header)
        else:
...
def _pptx_tables(data: bytes) -> dict[str, _t.Any]:
    """Extract PPTX tables as DataFrames with header inference per slide."""
    import pandas as pd
    from pptx import Presentation
    prs = Presentation(io.BytesIO(data))
    tables: dict[str, _t.Any] = {}
    for s_idx, slide in enumerate(prs.slides, 1):
        t_count = 0
        for shape in slide.shapes:
            # python-pptx exposes shape.has_table for table shapes
            if hasattr(shape, "has_table") and shape.has_table:
                t_count += 1
                tbl = shape.table
                rows = []
                # Access via row/column counts
                for r in range(len(tbl.rows)):
                    row_vals = []
                    for c in range(len(tbl.columns)):
                        row_vals.append(tbl.cell(r, c).text.strip())
                ...
                if all(h.strip() for h in header):
                    ...
                    df = pd.DataFrame(rows)
                    df.columns = [f"col_{i+1}" for i in range(len(df.columns))]
                tables[f"Slide {s_idx} • Table {t_count}"] = df
    return tables
...
class DocumentMatcher:
    ...
    def match(
        ...
        try:
            if detected == ".pdf":
                ...
                # Additionally search DOCX tables
                try:
                    docx_tbls = _docx_tables(data)
                    logger.info(f"DOCX tables found: {len(docx_tbls)}")
                    for name, df in docx_tbls.items():
                        sheet_matches = self._search_table(df, q, sheet=name)
                        matches.extend(sheet_matches)
                    if docx_tbls:
                        reason = "exact_cell_match" if matches else reason
                except Exception as e:
                    logger.debug(f"DOCX table extraction failed: {e}")
                ...
                # Additionally search PPTX tables
                try:
                    pptx_tbls = _pptx_tables(data)
                    logger.info(f"PPTX tables found: {len(pptx_tbls)}")
                    for name, df in pptx_tbls.items():
                        sheet_matches = self._search_table(df, q, sheet=name)
                        matches.extend(sheet_matches)
                    if pptx_tbls:
                        reason = "exact_cell_match" if matches else reason
                except Exception as e:
                    logger.debug(f"PPTX table extraction failed: {e}")
                ...
                try:
                    pdf_tbls = _pdf_tables(data)
                    logger.info(f"PDF tables found: {len(pdf_tbls)}")
                    for name, df in pdf_tbls.items():
                        sheet_matches = self._search_table(df, q, sheet=name)
                        matches.extend(sheet_matches)
                    if pdf_tbls:
                        reason = "exact_cell_match" if matches else reason
    ...
    def match_text(
        ...
        q = query.strip()
        meta: dict[str, _t.Any] = {"file_type": ".txt", "query": q, "text_length": len(full_text)}
        matches: list[MatchSpan] = []
        evidence: MatchSpan | None = None
        is_match = False
        reason = "no_match"
        ...
        try:
            matches = self._best_spans(full_text, q, top_k=top_k)
            reason = "keyword" if matches else "no_match"
            ...
            logger.error(f"Error during text matching: {exc}", exc_info=True)
            reason = "error"
            meta["error"] = str(exc)
